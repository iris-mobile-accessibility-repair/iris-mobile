...
def judgeCalss(color):
    class0Color1 = min(
        (color_dist(get_color_set.colorToRGB(color), get_color_set.colorToRGB(col)), col, "class0") for col in
        colorSet_reference.colorTriple0)
    class1Color1 = min(
        (color_dist(get_color_set.colorToRGB(color), get_color_set.colorToRGB(col)), col, "class1") for col in
        colorSet_reference.colorTriple1)
    classColor1 = min(class0Color1, class1Color1)
    # print(classColor1[2])
    if classColor1[2] == "class1":
...
def get_colorInDB(color2, color_simi, best_T,betst_alpha):
    ...
    if judgeCalss(color2) == 1:
        ...
        for co in color_simi:
            ...
            (r, g, b) = colorToRGB(co)
            (h, s, v) = rgb2hsv(r, g, b)
            if get_S_T(s) == tagS0 and abs(v - v0) < 0.5:
...
# get_changeColor2('', '#FF3BEB', '#91E0FF', 4.5)
#
...
# CO = '#9F2392'
# (r0, g0, b0) = colorToRGB(CO)
# (h0, s0, v0) = rgb2hsv(r0, g0, b0)
# print(h0, s0, v0)

# color_simi = ['#737373', '#212121', '#7E5BBF', '#202020', '#6F6F6F', '#2153AE', '#1175A1', '#585858', '#1C1C1C']
#
# color2 = '#FAFAFA'
# best_T = 'X'
# betst_alpha = 14
