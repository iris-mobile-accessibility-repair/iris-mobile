...
def find_colorSet(formerColor, colorSet):
    ...
    for cl in colorSet:
        ...
        for coPair in colorSet[cl]:
            if coPair == []:
                continue
            if get_color_set.con_contrast(formerColor[1], coPair[0]) <= 1.5 and get_color_set.con_contrast(formerColor[1],
                                                                                                         coPair[
                                                                                                             1]) >= 4.5:
                color_simi.append(coPair[1])
            elif get_color_set.con_contrast(formerColor[1], coPair[1]) <= 1.5 and get_color_set.con_contrast(
                    formerColor[1], coPair[0]) >= 4.5:
                color_simi.append(coPair[0])
    return list(set(color_simi))

def find_colorSet_class1(formerColor, colorSet, component_class):
    color_simi = []
    # print colorSet
...
def find_colorSet_class(formerColor, colorSet, component_class):
    ...
    for coPair in colorSet[component_class]:
        if coPair == []:
            continue
        if get_color_set.con_contrast(formerColor[1], coPair[0]) <= 1.5 and get_color_set.con_contrast(formerColor[1], coPair[1]) >= 4.5:
            color_simi.append(coPair[1])
        elif get_color_set.con_contrast(formerColor[1], coPair[1]) <= 1.5 and get_color_set.con_contrast(formerColor[1], coPair[0]) >= 4.5:
            color_simi.append(coPair[0])
...
# Iris2
def find_colorSet_class2(formerColor, colorSet, component_class):
    color_simi = []
    # print colorSet
    for cl in colorSet:
        for coPair in colorSet[cl]:
            if coPair == []:
                continue
            if get_color_set.con_contrast(formerColor[1], coPair[1]) >= 4.5:
                color_simi.append(coPair[1])
            if get_color_set.con_contrast(formerColor[1], coPair[0]) >= 4.5:
                color_simi.append(coPair[0])
    # print component_class
    # print list(set(color_simi))
    return list(set(color_simi))

def get_changeColor_self0(resultPath, APKName, id_colorSet):
    colorChange = {}
    ...
    return colorChange
# Iris2
def get_changeColor_self(resultPath, APKName, id_colorSet, act_T_Alpha):
    colorChange = {}
    # id_colorSet = get_id_inOneAPK(os.path.join(resultPath, APKName), APKName)[1]
    # print id_colorSet
    nonIssueColor = nonIssueColorSet.get_nonIssueColorSet(resultPath, APKName)
    ...
    for idC in id_colorSet:
        ...
        color2 = id_colorSet[idC][0][0]
        if id_colorSet[idC][2] in act_T_Alpha:
            # print "TTT: " + str(act_T_Alpha[id_colorSet[idC][2]])
            best_T = act_T_Alpha[id_colorSet[idC][2]][0]
            betst_alpha = act_T_Alpha[id_colorSet[idC][2]][1]
        else:
            best_T = 'N'
            betst_alpha = 0
        final_color = hsvRGB2.get_colorInDB(color2, color_simi, best_T, betst_alpha)
        colorChange[idC] = (final_color, id_colorSet[idC][0], id_colorSet[idC][1])
    return colorChange
...
def find_colorToChange_self0(resultPath, APKName, id_bound_colorSet):
    color_id = {}
    id_findInAct = []
...
# Iris2
def find_colorToChange_self(resultPath, APKName, id_bound_colorSet, act_T_Alpha):
    color_id = {}
    id_findInAct = []
    waitChangeColor_self = get_changeColor_self(resultPath, APKName, id_bound_colorSet, act_T_Alpha)
    print "waitChangeColor_self"
    print waitChangeColor_self
    for wcolor_id in waitChangeColor_self:
        # # print wcolor_id
        ...
        # ratio = wcolor[2]
        # finalColor = ifFit(color1, color2, color3, ratio)
        finalColor = waitChangeColor_self[wcolor_id][0]
        if finalColor != '':
            color_id[wcolor_id] = finalColor
        else:
            id_findInAct.append(wcolor_id)
...
def get_changeColor0(nonIssueColorAct, id_colorSet, componentClass, id):
    colorChange = {}
    # id_colorSet = get_id_inOneAPK(os.path.join(resultPath, APKName), APKName)[1]
    # print id_colorSet
    ...
    return colorChange
# Iris2
def get_changeColor(nonIssueColorAct, id_colorSet, componentClass, id, act_T_Alpha):
    colorChange = {}
    # id_colorSet = get_id_inOneAPK(os.path.join(resultPath, APKName), APKName)[1]
    # print id_colorSet
    # print nonIssueColorAct
    ...
    color2 = id_colorSet[id][0][0]
    if id_colorSet[id][2] in act_T_Alpha:
        # print "TTT: " + str(act_T_Alpha[id_colorSet[id][2]])
        best_T = act_T_Alpha[id_colorSet[id][2]][0]
        betst_alpha = act_T_Alpha[id_colorSet[id][2]][1]
    else:
        best_T = 'N'
        betst_alpha = 0
    final_color = hsvRGB2.get_colorInDB(color2, color_simi, best_T, betst_alpha)
    colorChange[id] = (final_color, id_colorSet[id][0], id_colorSet[id][1])
    return colorChange

# Iris2
def find_colorToChange(nonIssueColorAct, id_colorSet, componentClass, id, colorToChange_class, act_T_Alpha):
    color_id = {}
    waitChangeColor = get_changeColor(nonIssueColorAct, id_colorSet, componentClass, id, act_T_Alpha)
    ...
    for wcolor_id in waitChangeColor:
        ...
        ratio = wcolor[2]
        # finalColor = ifFit(color1, color2, color3, ratio)
        finalColor = waitChangeColor[wcolor_id][0]
        if finalColor != '':
            color_id[wcolor_id] = finalColor
            colorToChange_class[id] = finalColor
        else:
            finalColor = hsvRGB2.get_changeColor2(color1, color2, color3, ratio)
...
def solveImageContrast(id, txt, colorToChange, imageId_style, nonIssueColorAct, id_bound_colorSet, imageId_Name, id_class):
    ...
    #     if id not in colorToChange:
    #         # print id
    #         colorToC = find_colorToChange(nonIssueColorAct, id_bound_colorSet, componentClass, id, act_T_Alpha)
    #         # print colorToChange
    #         colorToChange[id] = colorToC[id]
    #         # print colorToChange
    #     # print txt
